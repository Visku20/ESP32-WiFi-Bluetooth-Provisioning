...
static void bluetooth_init(void)
{
    ...
    if (ret != ESP_OK && ret != ESP_ERR_INVALID_STATE) {
        ESP_LOGE(TAG, "BLE memory release failed: %s",
                 esp_err_to_name(ret));
    }
    ...
    ret = esp_bt_gap_set_device_name("ESP32_WIFI_SETUP");
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Device name failed");
        return;
    }

    ret = esp_spp_register_callback(bluetooth_callback);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "SPP callback registration failed");
        return;
    }
    ...
    // Make the ESP32 visible to Classic Bluetooth devices
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Could not enable discoverable mode");
        return;
    }
    ...
    ESP_LOGI(TAG, "Device name: ESP32_WIFI_SETUP");
}
